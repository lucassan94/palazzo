// Validadores compartilhados (Admin)
...
export function validarCPF(value) {
  if (!value) return { valido: false, formatado: '' }
  const digits = value.replace(/\D/g, '')
  if (digits.length !== 11) return { valido: false, formatado: value, mensagem: 'CPF deve ter 11 dígitos.' }
  if (/^(\d)\1{10}$/.test(digits)) return { valido: false, formatado: value, mensagem: 'CPF inválido (sequência repetida).' }
  let soma = 0
  ...
  return { valido: true, formatado: `${digits.substring(0, 3)}.${digits.substring(3, 6)}.${digits.substring(6, 9)}-${digits.substring(9)}`, digits }
}
